...
public class MyCloseHashImpl<K,V> implements MyHash<K,V> {
    ...
    public void put(K k, V v) {
        ...
        if (hashArray[position] == null){
            ...
            this.size++;
        } if (hashArray[position].isDeleted()){
            hashArray[position] = new HashEntry<>(k, v);
            this.size++;
        }
        else {
    ...
    public V get(K k) {
        ...
        int iterations = 0;
        int position = getPosition(k, iterations);
        while (!(hashArray[position] == null || iterations  > hashArray.length)) {
            if (hashArray[position].getKey().equals(k)==true){
                break;
            } else {
                iterations++;
                position = getPosition(k, iterations);
        ...
        if (hashArray[position]==null){
            return null;
        } else if ((iterations <= hashArray.length && !(hashArray[position].isDeleted()))){
    ...
    @Override
    public HashEntry<K,V>[] getValues() {
